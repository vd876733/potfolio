  ...
  return (
    <div className="flex flex-col gap-6 min-h-[400px]">
      <div className="flex items-center gap-3 border-b border-white/10 pb-4">
        ...
        <h2 className="text-2xl font-space-grotesk font-bold text-white">Knowledge Forge</h2>
      </div>
          ...
          {activeTab === "Technical Skills" && (
            ...
            >
              {Object.entries(skills).map(([category, items]) => (
                <div key={category} className="p-4 bg-black/20 rounded-xl border border-white/5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-white font-bold font-space-grotesk">{category}</h3>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {items.map((skill) => (
                      <span key={skill} className="px-2 py-1 bg-white/5 border border-white/10 text-white/80 text-xs font-inter rounded-md">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {certificates.map((cert, index) => (
                <div key={index} className="flex flex-col justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-zinc-700 transition-all">
                  <div>
                    ...
                    <h4 className="text-base font-bold text-white mt-1 mb-2">{cert.title || cert.name}</h4>
