...
export default function TopNav({ onNavClick, viewMode, toggleViewMode }: TopNavProps) {
  ...
  return (
    <div className="fixed top-0 inset-x-0 z-40 p-4 sm:p-8 pointer-events-none">
      <nav className="glass-panel w-full max-w-7xl mx-auto rounded-2xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4 pointer-events-auto border border-white/5 shadow-xl bg-obsidian/60">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-space-grotesk font-bold tracking-widest text-white uppercase hidden sm:block">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6">
          {navLinks.map((link) => (
            <button
              ...
              className="text-white/60 hover:text-white text-xs sm:text-sm font-inter transition-colors"
            >
              {link.label}
            </button>
          ))}
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-colors text-white text-xs sm:text-sm font-inter font-medium"
          >
